...
class View(APIView):
    def get(self, request):
        ...
        myPdno = set(Clientfollowstock.filter(clientid=request.user.id).values_list('symbol'))
        json.dumps(list(feeds), cls=DjangoJSONEncoder)
        feedLst=[]
        for rc in feeds:
            if rc['extr_stck_cd_list']:
                tmp = set(rc['extr_stck_cd_list'].split(',')
                if myPdno.isdisjoint(tmp): continue
                ...
                try:
                    ...
                    rc['heartYN'] = None
                feedLst.append(rc)
        return render(request, 'main_board.html', context  = dict(feedLst = feedLst))
