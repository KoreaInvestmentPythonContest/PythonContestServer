...
class View(APIView):
    def get(self, request):
        ...
        if request.user.is_authenticated:
            myPdno = set(Clientfollowstock.objects.filter(clientid=request.user.id).values_list('symbol'))
            feedLst=[]
            for rc in feeds:
                if rc['extr_stck_cd_list']:
                    ...
                    try:
                        ...
                        rc['heartYN'] = None
                    feedLst.append(rc)
            return render(request, 'main_board.html', context  = dict(feedLst = feedLst[:30]))
        else:
            for rc in feeds:
                if rc['extr_stck_cd_list']:
                    ...
                    try:
                        ...
                        rc['heartYN'] = None
        return render(request, 'main_board.html', context  = dict(feedLst = feeds[:21]))
